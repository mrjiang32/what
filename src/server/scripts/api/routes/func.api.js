...
const writeSuiteInnerFile = async ({ suiteName, fileName, raw, res }) => {
  ...
  if (fileName.endsWith(".js")) {
    try {
      ...
      return res
        .status(400)
        .json({ ok: false, error: "代码语法错误:" + err.message });
    }
  }
  try {
...
async function npmList(suiteDir) {
  const { stdout } = await execFileAsync("npm", ["list", "--json"], {
    cwd: suiteDir,
  });
...
export async function createModuleRoutes({ apiPrefix, source }) {
  ...
  const npmInstallRegex = new RegExp(
    `^${apiPrefix}/([a-zA-Z0-9_-]+)/npm/install$`,
  );
  const npmUninstallRegex = new RegExp(
    `^${apiPrefix}/([a-zA-Z0-9_-]+)/npm/uninstall$`,
  );
  ...
  return [
    ...
    // POST /api/func/:suiteName/exec 执行套件
    {
      ...
      handler: async (req, res) => {
        ...
        try {
          const execResult = await runSuiteModule(
            source,
            suiteName,
            req.body.params ?? {},
          );
    ...
    // POST /api/func/:suiteName/npm/install   body { pkgs:["lodash","moment"] }
    {
      ...
      handler: async (req, res) => {
        ...
        if (!Array.isArray(pkgs) || pkgs.length === 0) {
          return res
            .status(400)
            .json({ ok: false, error: "pkgs 需要非空数组" });
        }
        try {
